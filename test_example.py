...
async def demo_auto_eos():
    ...
    print(f"Auto-detected EOS tokens in trie: {getattr(beam.states[0].trie.trie, 'eos_tokens', 'None')}")
        ...
        if probs:
            ...
            if valid_bytes:
                ...
                if next_byte == 257:
                    ...
                    print(f"Generated byte: {next_byte} ({chr(next_byte) if 32 <= next_byte <= 126 else 'non-printable'})")
            else:
...
async def demo_manual_eos():
    ...
    params = BeamParams(K=3, prune_threshold=0.1, verbose=True, 
                       eos_tokens=eos_tokens, auto_eos=False)
    beam = await ByteBeamState.initial(model, params)
        ...
        if probs:
            ...
            if valid_bytes:
                ...
                if next_byte == 257:
                    ...
                    print(f"Generated: {chr(next_byte) if 32 <= next_byte <= 126 else f'byte({next_byte})'}")
            else:
                print("No valid bytes available (all keys are None)")
                break
        else:
...
async def demo_combined_eos():
    ...
    print(f"Should include both auto-detected '<|endoftext|>' and manual tokens {manual_eos}")
    ...
    for i in range(8):
        ...
        if probs:
            ...
            if valid_bytes:
                ...
                if next_byte == 257:
                    print("Combined EOS generated - could be from tokenizer or manual tokens!")
                    break
                else:
                    beam = await (beam.prune() << next_byte)
                    print(f"Generated: {chr(next_byte) if 32 <= next_byte <= 126 else f'byte({next_byte})'}")
