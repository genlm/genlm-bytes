...
def test_conditioning_mode_mass_distribution(eos_trie: TokenByteTrie):
...
def test_generation_mode_mass_distribution(eos_trie: TokenByteTrie):
    ...
    expected_mass = weights[3]  # Only P("!!")
    assert np.isclose(masses[node_for_exclamation], expected_mass.item())
...
def test_generation_mode_eos_node_aggregation(eos_trie: TokenByteTrie):
    ...
    expected_eos_mass = (
        weights[2] + weights[4] + weights[7]
    )  # P("!") + P(".") + P("</s>")
    actual_eos_mass = masses[eos_trie.eos_node]

    assert np.isclose(actual_eos_mass, expected_eos_mass.item())
